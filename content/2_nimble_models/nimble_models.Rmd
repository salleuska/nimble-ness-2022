...
`r icon_style(fontawesome("exclamation-circle"), fill = "darkred")` The two models are are not equivalent
...
littersInits <- list(mu = rep(0.5, 2), sigma = rep(1, 2))
...
model$simulate('p')  # simulate from prior
model$p
model$getLogProb('p')  # log prob not yet updated!
model$calculate('p')   # update it
model$getLogProb('p')  # now we're good
```
]
---
...
model$getLogProb('p')  
# recall why this hasn't changed yet?
```
...
## INSTEAD DO THIS
deps <- model$getDependencies(c('mu[1]','sigma[1]'))
...
The node `lifted_d1_over_sqrt_oPtau_cP` has been inserted between `tau` and `x`.   The resulting model would equivalently have been created by this model code:
